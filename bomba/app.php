<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $sqlStatement = $conn->prepare("SELECT * FROM bomba WHERE id_activacion = :id");
            ...
            $data = array();
            $sqlStatement = $conn->query("SELECT * FROM bomba");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            if($conn) {
                ...
                $sqlStatement->bindParam(':estado', $data->estado);
                $sqlStatement->bindParam(':idUsuario', $data->idUsuario);
                $sqlStatement->bindParam(':idTinaco', $data->idTinaco);
                $sqlStatement->execute();
                ...
                if($conn) {
                    ...
                    $sqlStatement = $conn->prepare(
                        "UPDATE bomba SET
                            ...
                            tinaco_id_tinaco = :idTinaco
                            WHERE id_activacion = :id"
                    );
